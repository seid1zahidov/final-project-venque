...
const Community2details = () => {
    ...
    useEffect(() => {
      axios.get('http://localhost:3002/Community2')
        .then(res => Setslider(res.data.com2))
  ...
  return (
    ...
    <div className="col-lg-10 cartdetail">
            ...
            {slider.map((row, __id) => (
              ...
              >
                ...
                <TableCell align="right">
                  <Link to={`/Com2update/${slider_arr[__id]}`} onClick={async () => {
                    await axios.put(`http://localhost:3002/Community2/${slider_arr[__id]}`)
                      .then(res => res.data)
                  }} className=" btn detailsbutton btn-warning" >Update</Link>
                </TableCell>
                <TableCell align="right">
                  <Link to={`/Admin/${slider_arr[__id]}`} onClick={async () => {
                    await axios.delete(`http://localhost:3002/Community2/${slider_arr[__id]}`)
      ...
      <Link to='/Com2Add' className=" btn btn-success Detailscom2 " >Add img</Link>
    </div>
  </div>
  )
}

export default Community2details