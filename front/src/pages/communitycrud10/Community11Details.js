...
const Community11Details = () => {
    ...
    useEffect(() => {
        axios.get('http://localhost:3002/Community12')
            .then(res => Setslider(res.data.com11))
  ...
  return (
    ...
    <div className="col-lg-10 cartdetail">
                    ...
                    {slider.map((row, __id) => (
                        ...
                        >
                            ...
                            <TableCell align="right">
                                <Link to={`/Com12update/${slider_arr[__id]}`} onClick={async () => {
                                    await axios.put(`http://localhost:3002/Community12/${slider_arr[__id]}`)
                                        .then(res => res.data)
                                }} className=" btn detailsbutton btn-warning" >Update</Link>
                            </TableCell>
                            <TableCell align="right">
                                <Link to={`/Admin/${slider_arr[__id]}`} onClick={async () => {
                                    await axios.delete(`http://localhost:3002/Community12/${slider_arr[__id]}`)
        ...
        <Link to='/Com12Add' className=" btn btn-success Detailscom2 " >Add img</Link>
    </div>
</div>
  )
}

export default Community11Details