...
const Community6updata = () => {
  ...
  useEffect(() => {
    const fetcHandler = async () => {
      await axios.get(`http://localhost:3002/Community6/${id}`)
        .then(res => res.data)
        .then(data => Setinputs(data.com6))
  ...
  const sendRequest = async () => {
    await axios.put(`http://localhost:3002/Community6/${id}`, {
      image: String(inputs.image),
    }).then(res => res.data.com6)
  }
...
export default Community6updata