...
const Community8update = () => {
  ...
  useEffect(() => {
    const fetcHandler = async () => {
      await axios.get(`http://localhost:3002/Community9/${id}`)
        .then(res => res.data)
        .then(data => Setinputs(data.com8))
  ...
  const sendRequest = async () => {
    await axios.put(`http://localhost:3002/Community9/${id}`, {
      image: String(inputs.image),
    }).then(res => res.data.com8)
  }
...
export default Community8update