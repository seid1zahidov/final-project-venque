...
import './Community5.css'
const Community5details = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:3002/Community5')
      .then(res => Setslider(res.data.com5))
  ...
  return (
    ...
    <div className="col-lg-10 cartdetail">
            ...
            {slider.map((row, __id) => (
              ...
              >
                ...
                <TableCell align="right">
                  <Link to={`/Com5update/${slider_arr[__id]}`} onClick={async () => {
                    await axios.put(`http://localhost:3002/Community5/${slider_arr[__id]}`)
                      .then(res => res.data)
                  }} className=" btn detailsbutton btn-warning" >Update</Link>
                </TableCell>
                <TableCell align="right">
                  <Link to={`/Admin/${slider_arr[__id]}`} onClick={async () => {
                    await axios.delete(`http://localhost:3002/Community5/${slider_arr[__id]}`)
      ...
      <Link to='/Com5Add' className=" btn btn-success Detailscom2 " >Add img</Link>
    </div>
  </div>
  )
}

export default Community5details