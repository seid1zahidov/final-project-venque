...
const Community5update = () => {
  ...
  useEffect(() => {
    const fetcHandler = async () => {
      await axios.get(`http://localhost:3002/Community5/${id}`)
        .then(res => res.data)
        .then(data => Setinputs(data.com5))
  ...
  const sendRequest = async () => {
    await axios.put(`http://localhost:3002/Community5/${id}`, {
      image: String(inputs.image),
    }).then(res => res.data.com5)
  }
...
export default Community5update