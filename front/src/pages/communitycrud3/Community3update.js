...
const Community3update = () => {
  ...
  useEffect(() => {
    const fetcHandler = async () => {
      await axios.get(`http://localhost:3002/Community3/${id}`)
        .then(res => res.data)
        .then(data => Setinputs(data.com3))
  ...
  const sendRequest = async () => {
    await axios.put(`http://localhost:3002/Community3/${id}`, {
      image: String(inputs.image),
    }).then(res => res.data.com3)
  }
...
export default Community3update